 ...
 */
extend('dataListConf',['$:@custom/treat/gridBtnOperation:gridBtnOperation'],function (gridBtnOperation) {

     // viewId = $_GET['viewId'];

    //顶部按钮组件的数据提取
    function extractTopBtn (btnList,gridApi,view) {
        ...
        if (btnList != null && btnList.length > 0) {
            for(var key in btnList){
                ...
                if (btndata.type != "1" && btndata.type != "6" && btndata.operationPosition == 1) {
                    if (btndata.type != "4" && btndata.status == '0' ) {
                     // var btn = gridBtnOperation(rowData,btndata,view,gridApi).eventsList;
                     var btn = {
                         ...
                         events:gridBtnOperation(rowData,btndata,view,gridApi)
                     }
                    }
                }
                var judgeShowOrHide = true;
                if (operationConditions != "" && operationConditions != null) {
                    ...
                    for (var k in operationConditions) {
                        ...
                        var meOperationRuleConditionList = operationConditionsChosen.operationRuleConditions[0];
                        ...
                        var conditionValue = meOperationRuleConditionList.conditionValue;
                            ...
                            if (affectOperationId == btndata.id) {
                                ...
                                // console.log(rowData,rowData[strs], conditionValue, conditionType)
                    ...
                    judgeShowOrHide = result;
                } else {
                    judgeShowOrHide = true;
                }
                // console.log(judgeShowOrHide,btn)
    ...
    function exportFunc(viewId,gridApiConf,gridApi) {
        ...
        var defaultSearchTagId = view && view.defaultSearchTagId ? view.defaultSearchTagId : 0;   //默认的查询标签，如果存在就渲染


            var gridConf;
        gridFieldInfo = {};
        //顶部按钮
        topBtn = extractTopBtn(btnList,gridApi,view);
        ...
        if(colList != null && colList != undefined && colList != ''){
            colList.forEach(function (menuInfo) {
                                ...
                                colModel.listConfig = function (data, colData) {
                                    ...
                                    switch (menuInfo.dateType) {
                                        ...
                                        case 'time':
                                            ...
                                            break;
                                        case 'datetime':
                                            temp = '{{data|Date:[$,"yy-mm-dd hh:ii"]}}';
                                            break;
                                        default:
                                            temp = '{{data|Date:[$,"yy-mm-dd"]}}';
                                            console.log('lalal啦啦啦时间')
                                    }
                ...
                if(menuInfo.phyColumnName == "C_USER_CODE"){
                    subjoin = "_UNAME"
                }
        ...
        var specialShowId = [];
        //遍历对比提取相关字段key 与条件信息
        if(specialShowdata != null && specialShowdata != undefined && specialShowdata != '') {
        ...
        //此处主要处理列表数据条件(颜色显示处理)
        resList[resList.length-1].listConfig = function(data, rowData, index ,gridListData,eles){
        ...
        gridConf = {
                ...
                //数据默认排序 [ asc  | desc ]
                order: orderData?orderData.order:'desc',
                //排序的字段
                // "orderField": orderData.sidx,
                "sendData": {
                    'tagId':defaultSearchTagId
                },
                //列表左边操作
                "leftColsModel": [
                    {
...
// console.log(developScope)
                                ...
                                scope: {
                                    ...
                                    onChange: function () {
                                        if (this.checked) {
                                            ...
                                            var checkBoxSet = document.querySelectorAll('input[type="checkbox"]');
                                            checkBoxSet.forEach(function(int){
                                                int.checked = true
                                            })

                                            //将所有的列表id出入到页面隐藏的input中
                                            var listData = developScope.gridListData.dataList;
                                            var dt = document.querySelector('input[name=ids]').value;
                                            listData.forEach(function(rel){
                                                document.querySelector('input[name=ids]').value = dt + "," + rel.ID;
                                            })
                                        } else {
                                            developScope.isAllChecked = false;
                                            developScope.allChecked = false;
                            ...
                            var developScope = this.developScope,
                                ...
                                scope = {
                                    ...
                                    onChange: function () {
                                        ...
                                        //点击每行选框将数据存储在空的input中

                                        var checkBoxSet = document.querySelectorAll('input[type="checkbox"]');
                                        var dt = document.querySelector('input[name=ids]').value;

                                        if (checkBoxSet[index + 1].checked == true) {
                                            document.querySelector('input[name=ids]').value = dt + "," + rowData.ID;

                                        } else {
                                            // ME.plugins.$("input[name=ids]").val(dt.replace("," + data.ID, ""));
                                            document.querySelector('input[name=ids]').value = dt.replace("," + rowData.ID, "");
                                        }
                ...
                filtration: function (data, callback) {
                    ...
                    $FRAME.server({
                        ...
                        "fuzzyQueryVal":data.fuzzyQueryVal||''
                    });
                             ...
                             if (viewDt.state == 0) {
                                ...
                                if (view.viewType == 5) {               //自定义
                                    if (view.viewUrl.indexOf('?') > 0) {
                                         $FRAME.redirect(view.viewUrl + "&recordId=" + recordId + '&cp=' + cp + "&meViewId=" + rowBtnForwardId_0 + "&moduleId=" + view.moduleId);
                                    } else {
                                         $FRAME.redirect(view.viewUrl + "?meViewId=" + rowBtnForwardId_0 + "&moduleId=" + view.moduleId +"&recordId=" + recordId + '&cp=' + cp);
                                    }
                                } else if (view.viewType == 4) {        //详情
                                     $FRAME.redirect('home/custom/detail?viewId=' + rowBtnForwardId_0 +"&moduleId=" + view.moduleId + "&recordId=" + recordId + '&cp=' + cp);
                                }
                            }
                        }.bind(this)).send({
                        ...
                        var $moduleListServer = this.server({
                            ...
                            url:'viewSearchDetail'
                             ...
                             if (viewDt.state == 0) {
                                ...
                                if (view.viewType == 5) {               //自定义
                                    if (view.viewUrl.indexOf('?') > 0) {
                                         $FRAME.redirect(view.viewUrl + "&recordId=" + recordId + '&cp=' + cp + "&viewId=" + rowBtnForwardId_0 + "&moduleId=" + view.moduleId );
                                    } else {
                                         $FRAME.redirect(view.viewUrl + "?viewId=" + rowBtnForwardId_0 + "&moduleId=" + view.moduleId +"&recordId=" + recordId + '&cp=' + cp);
                                    }
                                } else if (view.viewType == 4) {        //详情
                                     $FRAME.redirect('home/custom/detail?viewId=' + rowBtnForwardId_0 + "&moduleId=" + view.moduleId +"&recordId=" + recordId + '&cp=' + cp);
                                }
